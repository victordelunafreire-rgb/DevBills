...
export const initializeGlobalCategories = async (): Promise<Category[]> => {
	...
	for (const category of globalCategories) {
		try {
			...
			if (!existing) {
				const newCategroy = await prisma.category.create({ data: category });
				console.log(`✅ Criada: ${newCategroy.name}`);
				createdCategories.push(newCategroy);
			} else {
				createdCategories.push(existing);
			}
		} catch (err) {
			console.error(err, '🚨 Erro ao criar categoria');
		}
	}
